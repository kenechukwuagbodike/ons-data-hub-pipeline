...
def transform_data(df: pd.DataFrame) -> pd.DataFrame:
    """
    Transform the input DataFrame (eg transpose, clean, standardize).
    This function transposes the DataFrame so that time-series dates become rows,
    sets the first row as the header, renames the first column to 'date',

    Args:
        df (pd.DataFrame): Raw input DataFrame to be transformed.
